(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function a(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=a(n);fetch(n.href,o)}})();const q={},A="celpip_openai_api_key",H="celpip_openai_model",J="gpt-4o-mini";function I(){const e=localStorage.getItem(A);if(e)return e;const t=q==null?void 0:q.VITE_OPENAI_API_KEY;return t&&t!=="sk-your-key-here"?t:null}function K(e){e?localStorage.setItem(A,e.trim()):localStorage.removeItem(A)}function F(){localStorage.removeItem(A)}function W(){return localStorage.getItem(H)||J}function V(e){localStorage.setItem(H,e)}function Q(){return!!I()}async function X(e,t={}){var s;const a=I();if(!a)throw new Error("OpenAI API key is not configured.");const i=t.model||W(),n=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`},body:JSON.stringify({model:i,messages:e,temperature:t.temperature??.7,max_tokens:t.max_tokens??2048,...t.response_format?{response_format:t.response_format}:{}})});if(!n.ok){const d=await n.json().catch(()=>({}));throw new Error(((s=d.error)==null?void 0:s.message)||`OpenAI API error: ${n.status}`)}return(await n.json()).choices[0].message.content}async function $(e,t={}){const a=await X(e,{...t,response_format:{type:"json_object"}});return JSON.parse(a)}async function U(e,t={}){var o;const a=I();if(!a)throw new Error("OpenAI API key is not configured.");const i=await fetch("https://api.openai.com/v1/images/generations",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`},body:JSON.stringify({model:"dall-e-3",prompt:e,n:1,size:t.size||"1024x1024",quality:t.quality||"standard"})});if(!i.ok){const s=await i.json().catch(()=>({}));throw new Error(((o=s.error)==null?void 0:o.message)||`DALL-E API error: ${i.status}`)}return(await i.json()).data[0].url}async function Z(e){var o;const t=I();if(!t)throw new Error("OpenAI API key is not configured.");const a=new FormData;a.append("file",e,"recording.webm"),a.append("model","whisper-1"),a.append("language","en");const i=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${t}`},body:a});if(!i.ok){const s=await i.json().catch(()=>({}));throw new Error(((o=s.error)==null?void 0:o.message)||`Whisper API error: ${i.status}`)}return(await i.json()).text}async function ee(e,t={}){var o;const a=I();if(!a)throw new Error("OpenAI API key is not configured.");const i=await fetch("https://api.openai.com/v1/audio/speech",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`},body:JSON.stringify({model:"tts-1",input:e,voice:t.voice||"alloy",response_format:"mp3"})});if(!i.ok){const s=await i.json().catch(()=>({}));throw new Error(((o=s.error)==null?void 0:o.message)||`TTS API error: ${i.status}`)}const n=await i.blob();return URL.createObjectURL(n)}const L=[{id:1,name:"Writing an Email",description:"Write a formal or informal email responding to a given situation.",duration:1620,wordTarget:"150–200 words"},{id:2,name:"Responding to Survey Questions",description:"Choose an option and explain your decision in a written response.",duration:1560,wordTarget:"150–200 words"}];function te(e){var t;e.innerHTML=`
  ...
  `,v(),(t=document.getElementById("btn-back"))==null||t.addEventListener("click",()=>c("home")),e.querySelectorAll(".task-item").forEach(a=>{a.addEventListener("click",()=>{const i=parseInt(a.dataset.taskId);c("writing-practice",{taskId:i})})})}async function ae(e,t={}){var g,u;const a=t.taskId||1,i=L.find(l=>l.id===a)||L[0];e.innerHTML=`
    ${f()}
    <main class="container">
      ${y("Writing")}
  ...
  `,v(),(g=document.getElementById("btn-back"))==null||g.addEventListener("click",()=>c("writing"));let n;try{n=await ie(a)}catch{n=se(a)}let o;e.innerHTML=`
    ${f()}
    <main class="container">
  ...
  `;let o;try{o=await oe(t,a,i,n)}catch(p){e.innerHTML=`
    ...
    `,v(),(s=document.getElementById("btn-back"))==null||s.addEventListener("click",()=>c("writing")),(d=document.getElementById("btn-retry"))==null||d.addEventListener("click",()=>c("writing-practice",{taskId:t}));return}ne(e,t,a,i,o)}function ne(e,t,a,i,n){var d,p,g,u;L.find(l=>l.id===t);const o=L.find(l=>l.id===t+1);let s="";o&&(s=`<button class="btn btn-primary" id="btn-continue-task">Continue → Task ${o.id}: ${o.name}</button>`),e.innerHTML=`
    ${f()}
    <main class="container results-container">
      ${y("Writing")}
...
${a}`}],{max_tokens:3e3})}function se(e){return e===1?{prompt:"You recently moved to a new neighbourhood. Write an email to your neighbour introducing yourself. Include information about who you are, why you moved, and suggest a time to meet. Write approximately 150–200 words.",type:"informal"}:{prompt:"Your local community centre is considering extending its hours to include evenings and weekends. Do you think this is a good idea?",options:["Yes, the community centre should extend its hours.","No, the current hours are sufficient."]}}function re(e,t,a){try{const i=JSON.parse(localStorage.getItem("celpip_history")||"[]");i.push({section:e,taskId:t,score:a.overall_score,date:new Date().toISOString()}),localStorage.setItem("celpip_history",JSON.stringify(i))}catch{}}const E=[{id:1,name:"Giving Advice",description:"Offer advice on a personal situation.",prep:30,response:90},{id:2,name:"Talking About a Personal Experience",description:"Describe a past experience.",prep:30,response:60},{id:3,name:"Describing a Scene",description:"Describe what is happening in an AI-generated image.",prep:30,response:60,hasImage:!0},{id:4,name:"Making Predictions",description:"Based on the same scene image, predict what will happen next.",prep:30,response:60,hasImage:!0,usesTask3Image:!0},{id:5,name:"Comparing and Persuading",description:"Compare two options and persuade the listener.",prep:60,response:60},{id:6,name:"Dealing With a Difficult Situation",description:"Handle a challenging interpersonal scenario.",prep:60,response:60},{id:7,name:"Expressing Opinions",description:"Share and defend a viewpoint.",prep:30,response:90},{id:8,name:"Describing an Unusual Situation",description:"Explain an unexpected or unusual scenario shown in an image.",prep:30,response:60,hasImage:!0}];function le(e){var t;e.innerHTML=`
  ...
  `,v(),(d=document.getElementById("btn-back"))==null||d.addEventListener("click",()=>c("speaking"));let s;try{if(a===8){const p=await ue();s={prompt:p.prompt},n=p.imageUrl,o=p.sceneDescription}else{if(i.hasImage&&!n){const p=await me(a);n=p.imageUrl,o=p.sceneDescription}s=await ge(a,o)}}catch{s=ve(a)}ce(e,i,s,n,o)}function ce(e,t,a,i,n){var d,p;e.innerHTML=`
    ${f()}
    <main class="container">
      <div class="practice-header">
        <div>
  ...
  `,v(),(d=document.getElementById("btn-back"))==null||d.addEventListener("click",()=>c("speaking"));const o=document.getElementById("timer-display"),s=z(t.prep,g=>{o.innerHTML=k(g)},()=>{N(e,t,a,i,n)});(p=document.getElementById("btn-start-recording"))==null||p.addEventListener("click",()=>{s(),N(e,t,a,i,n)})}async function N(e,t,a,i,n){var l;let o,s=[],d=null;e.innerHTML=`
    ${f()}
    <main class="container">
      <div class="practice-header">
        <div>
  ...
  `;try{const r=await navigator.mediaDevices.getUserMedia({audio:!0});o=new MediaRecorder(r),s=[],o.ondataavailable=h=>{h.data.size>0&&s.push(h.data)},o.start()}catch{alert("Microphone access is required for speaking practice. Please allow microphone access in your browser settings."),c("speaking");return}const p=document.getElementById("timer-display"),g=z(t.response,r=>{p.innerHTML=k(r)},()=>{u()});function u(){g(),o&&o.state==="recording"&&o.stop(),o.onstop=()=>{d=new Blob(s,{type:"audio/webm"}),o.stream.getTracks().forEach(r=>r.stop()),pe(e,t,a,i,n,d)}}(l=document.getElementById("btn-stop-recording"))==null||l.addEventListener("click",u)}async function pe(e,t,a,i,n,o){var p,g,u,l,r;e.innerHTML=`
    ${f()}
    <main class="container">
      <div style="text-align: center; padding: var(--space-16) 0;">
        <div class="loading-spinner" style="width: 40px; height: 40px; margin: 0 auto var(--space-4);"></div>
  ...
  `;let s="";try{s=await Z(o)}catch{s=""}if(!s||s.trim().length===0){e.innerHTML=`
    ...
    <main class="container results-container">
      ...
      <!-- Suggestions -->
      <h3 style="font-size: var(--font-size-base); font-weight: 600; margin-bottom: var(--space-4);">💡 Improvement Suggestions</h3>
      ${(o.suggestions||[]).map((m,G)=>`
        <div class="suggestion-card">
          <div class="suggestion-header">Suggestion ${G+1} (${m.criterion})</div>
...
${a}`}],{max_tokens:3e3})}function ve(e){const t={1:{prompt:"Your friend is thinking about moving to a new city for work. They are nervous about starting over. Give them advice on how to settle in and make the transition smoother."},2:{prompt:"Describe a memorable trip you took. Where did you go, what did you do, and why was it special?"},3:{prompt:"Look at the image and describe what you see. Include details about the people, activities, objects, and setting."},4:{prompt:"Look at the image. Based on what you see happening in this scene, what do you think will happen in the next few minutes? Make predictions about the people, their actions, and the situation."},5:{prompt:"A friend is deciding between joining a gym or exercising outdoors. Compare both options and try to persuade them to choose one."},6:{prompt:"You ordered furniture online, but the wrong items were delivered. Call the customer service line to explain the problem and ask for a resolution."},7:{prompt:"Some people think remote work is better than working in an office. Do you agree or disagree? Explain your opinion."},8:{prompt:"You arrive at the office on Monday morning and find that all the furniture has been rearranged overnight. Describe the situation to your colleague and discuss what might have happened."}};return t[e]||t[1]}function ye(e,t,a){try{const i=JSON.parse(localStorage.getItem("celpip_history")||"[]");i.push({section:e,taskId:t,score:a.overall_score,date:new Date().toISOString()}),localStorage.setItem("celpip_history",JSON.stringify(i))}catch{}}const S=[{id:1,name:"Reading Correspondence",description:"Understand written correspondence (e.g., emails, letters).",questions:11},{id:2,name:"Reading to Apply a Diagram",description:"Interpret diagrams, tables, or visual information.",questions:9},{id:3,name:"Reading for Information",description:"Extract key details from informational passages.",questions:9},{id:4,name:"Reading for Viewpoints",description:"Analyse and compare opinions or viewpoints in written texts.",questions:10}],be=3300;function we(e){var t;e.innerHTML=`
  ...
  `,v(),(t=document.getElementById("btn-back"))==null||t.addEventListener("click",()=>c("home")),e.querySelectorAll(".task-item").forEach(a=>{a.addEventListener("click",()=>{const i=parseInt(a.dataset.taskId);c("reading-practice",{taskId:i})})})}async function ke(e,t={}){var g,u;const a=t.taskId||1,i=S.find(l=>l.id===a)||S[0],n=Math.floor(be/4);e.innerHTML=`
    ${f()}
    <main class="container">
      ${y("Reading")}
      <div style="text-align: center; padding: var(--space-16) 0;">
  ...
  `,v(),(g=document.getElementById("btn-back"))==null||g.addEventListener("click",()=>c("reading"));let o;try{o=await xe(a)}catch{o=$e()}const s={};e.innerHTML=`
    ${f()}
    <main class="container">
      <div class="practice-header">
        <div>
  ...
  `,v(),(u=document.getElementById("btn-back"))==null||u.addEventListener("click",()=>{p&&p(),c("reading")}),e.querySelectorAll('input[type="radio"]').forEach(l=>{l.addEventListener("change",r=>{const h=r.target.name;s[h]=parseInt(r.target.value)})});const d=document.getElementById("timer-display"),p=z(n,l=>{d.innerHTML=k(l)},()=>{_(e,a,o,s)});document.getElementById("btn-submit").addEventListener("click",()=>{p(),_(e,a,o,s)})}function _(e,t,a,i){var p,g,u,l;S.find(r=>r.id===t);let n=0;const o=a.questions.map((r,h)=>{const b=i[`q${h}`],m=b===r.correct;return m&&n++,{...r,userAnswer:b,isCorrect:m,index:h}}),s=Math.round(n/a.questions.length*9+3),d=Math.min(12,Math.max(3,s));e.innerHTML=`
    ${f()}
    <main class="container results-container">
      ${y("Reading")}
...
Neighbourhood Association President`,questions:[{question:"What is the main purpose of this letter?",options:["To complain about noise","To inform neighbours about a new project","To invite people to a party","To request volunteers"],correct:1,explanation:"The letter informs neighbours about the new community centre construction."},{question:"When will the community centre open?",options:["Next month","This Saturday","Next summer","Next year"],correct:2,explanation:"The passage states it is 'expected to open by next summer'."},{question:"What will NOT be included in the community centre?",options:["Swimming pool","Gymnasium","Restaurant","Library"],correct:2,explanation:"The passage mentions a pool, gym, meeting rooms, and library — but no restaurant."},{question:"When will construction noise occur?",options:["All day every day","Weekdays 7 a.m. to 6 p.m.","Only on weekends","Only at night"],correct:1,explanation:"The city assured work will be between 7:00 a.m. and 6:00 p.m. on weekdays."}]}}function Ee(e,t,a){try{const i=JSON.parse(localStorage.getItem("celpip_history")||"[]");i.push({section:e,taskId:t,score:a.overall_score,date:new Date().toISOString()}),localStorage.setItem("celpip_history",JSON.stringify(i))}catch{}}const T=[{id:1,name:"Listening to Problem Solving",description:"Understand a conversation about a problem and determine the best solution.",questions:8},{id:2,name:"Listening to a Daily Life Conversation",description:"Comprehend a conversation about everyday topics.",questions:5},{id:3,name:"Listening for Information",description:"Extract specific details from an informational dialogue.",questions:6},{id:4,name:"Listening to a News Item",description:"Understand the main ideas and details from a news-style report.",questions:6},{id:5,name:"Listening to a Discussion",description:"Follow views and opinions in a discussion.",questions:6},{id:6,name:"Listening to Viewpoints",description:"Identify and compare opinions from multiple speakers.",questions:7}];function Se(e){var t;e.innerHTML=`
  ...
  `,v(),(t=document.getElementById("btn-back"))==null||t.addEventListener("click",()=>c("home")),e.querySelectorAll(".task-item").forEach(a=>{a.addEventListener("click",()=>{const i=parseInt(a.dataset.taskId);c("listening-practice",{taskId:i})})})}async function Ie(e,t={}){var s;const a=t.taskId||1,i=T.find(d=>d.id===a)||T[0];e.innerHTML=`
    ${f()}
    <main class="container">
      ${y("Listening")}
      <div style="text-align: center; padding: var(--space-16) 0;">
  ...
  `,v(),(s=document.getElementById("btn-back"))==null||s.addEventListener("click",()=>c("listening"));let n,o=null;try{n=await Te(a),document.getElementById("loading-status").textContent="Generating audio...",o=await ee(n.dialogue_text||n.passage)}catch{n=ze()}Le(e,i,n,o)}function Le(e,t,a,i){var l;const n={};e.innerHTML=`
    ${f()}
    <main class="container">
      <div class="practice-header">
        <div>
  ...
  `,v(),(l=document.getElementById("btn-back"))==null||l.addEventListener("click",()=>{u&&u(),c("listening")});let s=!1;const d=document.getElementById("btn-play-audio"),p=document.getElementById("listening-audio");d&&p&&d.addEventListener("click",()=>{s||(s=!0,p.play(),d.disabled=!0,d.textContent="🔊 Playing...",d.classList.remove("btn-primary"),d.classList.add("btn-secondary"),document.getElementById("audio-status").textContent="Audio is playing...",p.addEventListener("ended",()=>{d.textContent="✓ Audio finished",document.getElementById("audio-status").textContent="Audio has been played (cannot replay)"}))}),e.querySelectorAll('input[type="radio"]').forEach(r=>{r.addEventListener("change",h=>{n[h.target.name]=parseInt(h.target.value)})});const g=document.getElementById("timer-display"),u=z(600,r=>{g.innerHTML=k(r)},()=>{D(e,t.id,a,n)});document.getElementById("btn-submit").addEventListener("click",()=>{u(),D(e,t.id,a,n)})}function D(e,t,a,i){var p,g,u,l;let n=0;const o=a.questions.map((r,h)=>{const b=i[`q${h}`],m=b===r.correct;return m&&n++,{...r,userAnswer:b,isCorrect:m}}),s=Math.round(n/a.questions.length*9+3),d=Math.min(12,Math.max(3,s));e.innerHTML=`
    ...
    <main class="container results-container">
      ...
      <div style="display: flex; gap: var(--space-3); margin-top: var(--space-6); justify-content: center; flex-wrap: wrap;">
        ...
        <button class="btn btn-secondary" id="btn-retry">Practise Again</button>
        ${(()=>{const r=T.find(h=>h.id===t+1);return r?'<button class="btn btn-primary" id="btn-continue-task">Continue → Part '+(t+1)+": "+r.name+"</button>":""})()}
      </div>
    </main>
  `,v(),(p=document.getElementById("btn-back"))==null||p.addEventListener("click",()=>c("listening")),(g=document.getElementById("btn-back-listening"))==null||g.addEventListener("click",()=>c("listening")),(u=document.getElementById("btn-retry"))==null||u.addEventListener("click",()=>c("listening-practice",{taskId:t})),(l=document.getElementById("btn-continue-task"))==null||l.addEventListener("click",()=>c("listening-practice",{taskId:t+1})),Pe("listening",t,{overall_score:d})}async function Te(e){const t=T.find(i=>i.id===e),a=`You are a CELPIP exam question writer. Generate a Listening Part ${e} (${t.name}) exercise.
...
}`;return await $([{role:"system",content:a},{role:"user",content:`Generate a new CELPIP listening exercise for Part ${e}.`}],{max_tokens:3e3})}function ze(e){return{dialogue_text:`Speaker 1: Hi, I'm having trouble with my internet connection. It's been really slow for the past few days.
...
Speaker 1: That's perfect. Thank you very much.`,passage:null,questions:[{question:"What is the caller's problem?",options:["Their phone is broken","Their internet is slow","They need a new modem","Their bill is too high"],correct:1,explanation:"The caller says their internet connection has been really slow."},{question:"What caused the problem?",options:["A storm","Maintenance work","A broken modem","An unpaid bill"],correct:1,explanation:"The agent mentions maintenance work in the area last week."},{question:"What solution does the caller choose?",options:["Remote modem reset","A technician visit","Cancelling the service","Upgrading the plan"],correct:1,explanation:"The caller says they'd prefer someone to come and check it."},{question:"When is the technician available?",options:["Monday morning","Wednesday evening","Thursday afternoon","Friday morning"],correct:2,explanation:"The agent offers Thursday afternoon between 2 and 5 p.m."}]}}function Pe(e,t,a){try{const i=JSON.parse(localStorage.getItem("celpip_history")||"[]");i.push({section:e,taskId:t,score:a.overall_score,date:new Date().toISOString()}),localStorage.setItem("celpip_history",JSON.stringify(i))}catch{}}const x=document.getElementById("app"),w={};let j=null;function c(e,t={}){window.history.pushState({path:e,params:t},"",`#${e}`),C(e,t)}function C(e,t={}){j=e;const a=w[e];a?a(t):R()}window.addEventListener("popstate",e=>{const t=e.state||{};C(t.path||"home",t.params||{})});w.home=()=>R();w.settings=()=>M();w.writing=()=>te(x);w["writing-practice"]=e=>ae(x,e);w.speaking=()=>le(x);w["speaking-practice"]=e=>de(x,e);w.reading=()=>we(x);w["reading-practice"]=e=>ke(x,e);w.listening=()=>Se(x);w["listening-practice"]=e=>Ie(x,e);function f(){return`
  ...
  `}function R(){var t,a,i,n,o;const e=!Q();x.innerHTML=`
    ${f()}
    <main class="container">
      ${e?`
  ...
  `,document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add("active")),n.addEventListener("click",s=>{s.target===n&&P(n)}),document.getElementById("settings-cancel").addEventListener("click",()=>P(n)),document.getElementById("settings-save").addEventListener("click",()=>{const s=document.getElementById("settings-api-key").value,d=document.getElementById("settings-model").value;K(s),V(d),P(n),C(j||"home")}),(o=document.getElementById("settings-remove"))==null||o.addEventListener("click",()=>{F(),P(n),C(j||"home")})}function P(e){e.classList.remove("active"),setTimeout(()=>e.remove(),300)}function v(){var e,t,a;(e=document.getElementById("nav-home"))==null||e.addEventListener("click",()=>c("home")),(t=document.getElementById("nav-settings"))==null||t.addEventListener("click",()=>M()),(a=document.getElementById("nav-history"))==null||a.addEventListener("click",()=>Y())}function Y(){var o;const e=document.querySelector(".modal-overlay");e&&e.remove();const t=JSON.parse(localStorage.getItem("celpip_history")||"[]").reverse(),a={writing:"✏️ Writing",speaking:"🎤 Speaking",reading:"📖 Reading",listening:"🎧 Listening"},i={writing:{1:"Writing an Email",2:"Survey Response"},speaking:{1:"Giving Advice",2:"Personal Experience",3:"Describing a Scene",4:"Making Predictions",5:"Comparing & Persuading",6:"Difficult Situation",7:"Expressing Opinions",8:"Unusual Situation"},reading:{1:"Correspondence",2:"Apply a Diagram",3:"For Information",4:"For Viewpoints"},listening:{1:"Problem Solving",2:"Daily Life",3:"For Information",4:"News Item",5:"Discussion",6:"Viewpoints"}},n=document.createElement("div");n.className="modal-overlay",n.innerHTML=`
  ...
  `,document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add("active")),n.addEventListener("click",s=>{s.target===n&&B(n)}),document.getElementById("history-close").addEventListener("click",()=>B(n)),(o=document.getElementById("history-clear"))==null||o.addEventListener("click",()=>{localStorage.removeItem("celpip_history"),B(n),Y()})}function B(e){e.classList.remove("active"),setTimeout(()=>e.remove(),300)}function y(e="Back"){return`<button class="btn btn-ghost" id="btn-back">← ${e}</button>`}function k(e){const t=Math.floor(e/60),a=e%60;return`<div class="timer ${e<60?"danger":e<120?"warning":""}">⏱ ${String(t).padStart(2,"0")}:${String(a).padStart(2,"0")}</div>`}function z(e,t,a){let i=e;t(i);const n=setInterval(()=>{i--,t(i),i<=0&&(clearInterval(n),a())},1e3);return()=>clearInterval(n)}R();
